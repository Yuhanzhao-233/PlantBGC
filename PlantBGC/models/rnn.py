...
import os
import pickle
from .transformer_layers import AddSinusoidalPositionalEncoding, LayerNorm, MultiHeadSelfAttention
...
class KerasRNN(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X_list, y_list, timesteps=128, validation_size=0.33, num_epochs=10, verbose=1,
            ...
            validation_X_list=None, validation_y_list=None, init_model_path=None, **kwargs):
        """
        ...
        # ===== Stage2 init: load pretrained weights from Stage1 .pkl =====
        if init_model_path:
            import os, pickle
            if not os.path.exists(init_model_path):
                raise ValueError("init_model_path not found: {}".format(init_model_path))

            with open(init_model_path, "rb") as f:
                pretrained_obj = pickle.load(f)

            # pretrained_obj 一般是 KerasRNN/KerasTransformer wrapper
            pretrained_keras = None
            if hasattr(pretrained_obj, "model") and hasattr(pretrained_obj.model, "get_weights"):
                pretrained_keras = pretrained_obj.model

            if pretrained_keras is None:
                raise ValueError("Cannot locate keras model inside pkl: {}".format(init_model_path))

            pre_w = pretrained_keras.get_weights()

            # ✅ 加一条“确认加载成功”的日志：打印第一层权重的均值
            import numpy as np, logging
            logging.info("Init model: %s, first weight mean=%.6f",
                         init_model_path, float(np.mean(pre_w[0])))

            try:
                train_model.set_weights(pre_w)
                self.model.set_weights(pre_w)
                logging.info("Loaded pretrained weights from: %s", init_model_path)
            except Exception as e:
                raise ValueError("Failed to set pretrained weights from {}: {}".format(init_model_path, e))
    ...
    def __setstate__(self, state):
        ...
        if architecture is None:
            ...
            self.model = model_from_json(architecture, custom_objects=custom_objects)
